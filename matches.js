...
let userJoinedTournamentIds = new Set();
let joinRequestsUnsubscribe = null;

// Secure session observer tracking dynamic user participation bounds
onAuthStateChanged(auth, (user) => {
  if (joinRequestsUnsubscribe) {
    joinRequestsUnsubscribe();
    joinRequestsUnsubscribe = null;
  }
  userJoinedTournamentIds.clear();
  ...
  if (user) {
    ...
    joinRequestsUnsubscribe = onSnapshot(q, (snap) => {
      userJoinedTournamentIds.clear();
      snap.forEach(docSnap => {
        const data = docSnap.data();
        if (data.tournamentId) {
          userJoinedTournamentIds.add(data.tournamentId);
        }
      });
      window.renderCategorizedTournaments();
    }, (err) => {
      console.error("Secure join state observer failed:", err);
    });
  } else {
    window.renderCategorizedTournaments();
  }
...
window.renderCategorizedTournaments = function() {
  ...
  filtered.forEach((t, i) => {
    ...
    // Dynamic Winner label extraction
    const winnerVal = t.winnerType === 'Custom' ? (t.winnerCustom || 'Custom') : (t.winnerType || '1st Place Only');

    // Secure verification of parameters based on individual publish indicators
    let roomIdDisplay = '🔒 Hidden';
    let roomPassDisplay = '🔒 Hidden';

    if (isAdminView) {
      roomIdDisplay = t.roomId || 'Not Set';
      roomPassDisplay = t.roomPass || 'Not Set';
    } else if (userJoinedTournamentIds.has(t.id)) {
      roomIdDisplay = t.roomIdPublished ? (t.roomId || 'Not Set') : 'Waiting for Admin';
      roomPassDisplay = t.roomPasswordPublished ? (t.roomPass || 'Not Set') : 'Waiting for Admin';
    }
      ...
      <div class="flex gap-0 min-h-0">
        ...
        <div class="flex-1 min-w-0 p-4 flex flex-col justify-between gap-3">
          <div>
            ...
            <p class="text-[9px] text-yellow-400 mt-0.5 font-bold flex items-center gap-1">
              <i class="fa-solid fa-trophy text-yellow-500"></i> Winner: ${winnerVal}
            </p>
          </div>
